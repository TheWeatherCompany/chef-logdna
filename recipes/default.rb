...
if node['logdna']['agent_install']
  ...
  begin
    ...
    when 'debian', 'ubuntu', 'linuxmint'
      include_recipe 'twc-chef-logdna::install_debian'
    when 'redhat', 'centos', 'amazon'
      include_recipe 'twc-chef-logdna::install_redhat'
    else
...
if node['logdna']['agent_configure']
  ...
  begin
    include_recipe 'twc-chef-logdna::configure'
...
if node['logdna']['conf_key'] || node['logdna']['agent_service'] == :stop
  ...
  begin
    case node['platform']
    when 'debian', 'ubuntu', 'linuxmint'
      include_recipe 'twc-chef-logdna::service_debian'
    when 'redhat', 'centos', 'amazon'
      include_recipe 'twc-chef-logdna::service_redhat'
    else
      raise 'Unsupported platform: #{node["platform"]}!'
    end
